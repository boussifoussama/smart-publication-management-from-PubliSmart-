    ...
    , arduinoBridge(nullptr)
{
    ...
    equipModel = new QSqlQueryModel(this);
    setupEquipementPage();
...
void SmartMarket::setupEquipementPage()
{
    // ── Sidebar navigation ──────────────────────────────────────
    connect(ui->equip_btnNavPub, &QPushButton::clicked, this, [this]() {
        ui->stackedWidgetMain->setCurrentIndex(1);
    });
    connect(ui->equip_btnNavConf, &QPushButton::clicked, this, [this]() {
        ui->stackedWidgetMain->setCurrentIndex(2);
        loadConferenceTable();
    });
    connect(ui->equip_btnNavEquip, &QPushButton::clicked, this, [this]() {
        ui->stackedWidgetMain->setCurrentIndex(3);
        loadEquipementTable();
    });

    // ── CRUD buttons ────────────────────────────────────────────
    connect(ui->equip_btnAjouter,   &QPushButton::clicked, this, &SmartMarket::equip_onAjouter);
    connect(ui->equip_btnModifier,  &QPushButton::clicked, this, &SmartMarket::equip_onModifier);
    connect(ui->equip_btnSupprimer, &QPushButton::clicked, this, &SmartMarket::equip_onSupprimer);
    connect(ui->equip_btnReset,     &QPushButton::clicked, this, &SmartMarket::equip_onReset);

    // ── Search / sort ───────────────────────────────────────────
    connect(ui->equip_btnRechercher, &QPushButton::clicked, this, &SmartMarket::equip_onRechercher);
    connect(ui->equip_btnTrierAsc,   &QPushButton::clicked, this, &SmartMarket::equip_onTrierAsc);
    connect(ui->equip_btnTrierDesc,  &QPushButton::clicked, this, &SmartMarket::equip_onTrierDesc);

    // Live filtering as user types
    connect(ui->equip_lineEdit_searchNom, &QLineEdit::textChanged,
            this, &SmartMarket::equip_onRechercher);
    connect(ui->equip_comboBox_searchType,
            QOverload<int>::of(&QComboBox::currentIndexChanged),
            this, &SmartMarket::equip_onRechercher);

    // ── Export / Stats ──────────────────────────────────────────
    connect(ui->equip_btnExcel, &QPushButton::clicked, this, &SmartMarket::equip_onExporterExcel);
    connect(ui->equip_btnStats, &QPushButton::clicked, this, &SmartMarket::equip_onShowStats);

    // ── Row click fills the form ────────────────────────────────
    connect(ui->equip_tableView, &QTableView::clicked,
            this, &SmartMarket::equip_onTableClicked);

    ui->equip_tableView->setItemDelegate(new StockColorDelegate(this));

    // Set date picker default to today
    ui->equip_dateEdit_achat->setDate(QDate::currentDate());

    loadEquipementTable();
    // ── Arduino setup ───────────────────────────────────────────
    arduinoEquip = new ArduinoEquipement(this);

    connect(arduinoEquip, &ArduinoEquipement::equipementUtilise,
            this, &SmartMarket::equip_onArduinoEvent);

    connect(arduinoEquip, &ArduinoEquipement::arduinoReady, this, [this]() {
        QMessageBox::information(this, "Arduino",
                                 "✅ Arduino connecté et prêt !");
    });

    connect(arduinoEquip, &ArduinoEquipement::erreurConnexion,
            this, [this](const QString &msg) {
                QMessageBox::warning(this, "Arduino", "Erreur :\n" + msg);
            });

    connect(ui->equip_btnArduino, &QPushButton::clicked,
            this, &SmartMarket::equip_onConnecterArduino);

    // Delegate coloration stock
    ui->equip_tableView->setItemDelegate(new StockColorDelegate(this));
}
...
// LOAD TABLE
// ================================================================
void SmartMarket::loadEquipementTable(QSqlQueryModel *model)
{
    if (!model) {
        Equipement e;
        model = e.afficher();
    }
    // Safe delete of previous model
    QAbstractItemModel *old = ui->equip_tableView->model();
    ui->equip_tableView->setModel(model);
    if (old && old != equipModel) delete old;
    equipModel = model;

    ui->equip_tableView->horizontalHeader()->setStretchLastSection(true);
    ui->equip_tableView->resizeColumnsToContents();
    updateEquipStatsBar();
}
...
// FORM HELPERS
// ================================================================
void SmartMarket::clearEquipementForm()
{
    ui->equip_lineEdit_id->clear();
    ui->equip_lineEdit_nom->clear();
    ui->equip_comboBox_type->setCurrentIndex(0);
    ui->equip_comboBox_etat->setCurrentIndex(0);
    ui->equip_spinBox_quantite->setValue(1);
    ui->equip_dateEdit_achat->setDate(QDate::currentDate());
    ui->equip_doubleSpinBox_prix->setValue(0.0);
    ui->equip_lineEdit_idConf->clear();
}

// Column indices in QSqlQueryModel (matches afficher() SELECT order):
//  0=idequipement  1=nom  2=type  3=etat  4=quantite
//  5=dateachat     6=prix 7=idconference
void SmartMarket::equip_onTableClicked(const QModelIndex &index)
{
    if (!equipModel) return;
    int row = index.row();

    ui->equip_lineEdit_id->setText(equipModel->index(row, 0).data().toString());
    ui->equip_lineEdit_nom->setText(equipModel->index(row, 1).data().toString());

    int typeIdx = ui->equip_comboBox_type->findText(equipModel->index(row, 2).data().toString());
    if (typeIdx >= 0) ui->equip_comboBox_type->setCurrentIndex(typeIdx);

    int etatIdx = ui->equip_comboBox_etat->findText(equipModel->index(row, 3).data().toString());
    if (etatIdx >= 0) ui->equip_comboBox_etat->setCurrentIndex(etatIdx);

    ui->equip_spinBox_quantite->setValue(equipModel->index(row, 4).data().toInt());

    QDate d = equipModel->index(row, 5).data().toDate();
    ui->equip_dateEdit_achat->setDate(d.isValid() ? d : QDate::currentDate());

    ui->equip_doubleSpinBox_prix->setValue(equipModel->index(row, 6).data().toDouble());
    ui->equip_lineEdit_idConf->setText(equipModel->index(row, 7).data().toString());

    // Show conference cost in the status bar at the bottom
    bool ok = false;
    int idConf = equipModel->index(row, 7).data().toInt(&ok);
    if (ok && idConf > 0) {
        Equipement e;
        double cout = e.coutTotalParConference(idConf);
        ui->equip_labelCoutConf->setText(
            QString("Coût conférence #%1 : %2 TND").arg(idConf).arg(cout, 0, 'f', 2));
    } else {
        ui->equip_labelCoutConf->setText("Coût conférence sélectionnée : —");
    }
}
...
// CRUD SLOTS
// ================================================================
void SmartMarket::equip_onAjouter()
{
    bool idOk = false;
    int id = ui->equip_lineEdit_id->text().trimmed().toInt(&idOk);
    if (!idOk || id <= 0) {
        QMessageBox::warning(this, "Erreur", "L'ID doit être un entier positif.");
        return;
    }
    const QString nom = ui->equip_lineEdit_nom->text().trimmed();
    if (nom.isEmpty()) {
        QMessageBox::warning(this, "Erreur", "Le nom est obligatoire.");
        return;
    }
    // ── Validation quantité ─────────────────────────────────────
    if (ui->equip_spinBox_quantite->value() < 0) {
        QMessageBox::warning(this, "Validation", "La quantité ne peut pas être négative.");
        return;
    }

    // ── Validation prix ─────────────────────────────────────────
    if (ui->equip_doubleSpinBox_prix->value() <= 0.0) {
        QMessageBox::warning(this, "Validation", "Le prix doit être supérieur à 0 TND.");
        return;
    }

    // ── Alerte stock faible (avant insertion) ───────────────────
    int qte = ui->equip_spinBox_quantite->value();
    if (qte == 0) {
        QMessageBox::warning(this, "⚠️ Rupture de stock",
                             "Cet équipement sera en rupture de stock (quantité = 0).");
    } else if (qte <= 5) {
        QMessageBox::warning(this, "⚠️ Stock faible",
                             QString("Stock faible : seulement %1 unité(s) disponible(s).").arg(qte));
    }

    bool confOk = false;
    int idConf = ui->equip_lineEdit_idConf->text().trimmed().toInt(&confOk);

    Equipement e(
        id,
        nom,
        ui->equip_comboBox_type->currentText(),
        ui->equip_comboBox_etat->currentText(),
        ui->equip_spinBox_quantite->value(),
        ui->equip_dateEdit_achat->date(),
        ui->equip_doubleSpinBox_prix->value(),
        confOk ? idConf : 0
        );
    ...
    if (e.ajouter()) {
        ...
        QMessageBox::critical(this, "Erreur",
                              "Échec de l'ajout.\nCauses possibles :\n"
                              "• ID déjà utilisé (doublon)\n"
                              "• ID Conférence inexistant\n"
                              "• Problème de connexion DB");
    }
}

void SmartMarket::equip_onModifier()
{
    bool idOk = false;
    int id = ui->equip_lineEdit_id->text().trimmed().toInt(&idOk);
    if (!idOk || id <= 0) {
        QMessageBox::warning(this, "Erreur",
                             "Cliquez d'abord sur une ligne du tableau pour sélectionner un équipement.");
        return;
    }
    const QString nom = ui->equip_lineEdit_nom->text().trimmed();
    if (nom.isEmpty()) {
...
void SmartMarket::equip_onSupprimer()
{
    bool idOk = false;
    int id = ui->equip_lineEdit_id->text().trimmed().toInt(&idOk);
    if (!idOk || id <= 0) {
    ...
    const QString nom = ui->equip_lineEdit_nom->text();
    auto ans = QMessageBox::question(this, "Confirmer suppression",
                                     QString("Supprimer l'équipement '%1' (ID %2) ?").arg(nom).arg(id),
...
void SmartMarket::equip_onReset()
{
    clearEquipementForm();
    ui->equip_lineEdit_searchNom->clear();
    ui->equip_comboBox_searchType->setCurrentIndex(0);
    ui->equip_labelCoutConf->setText("Coût conférence sélectionnée : —");
    loadEquipementTable();
}
...
// SEARCH / SORT
// ================================================================
void SmartMarket::equip_onRechercher()
{
    const QString nom  = ui->equip_lineEdit_searchNom->text().trimmed();
    const QString type = ui->equip_comboBox_searchType->currentText();
    Equipement e;
    loadEquipementTable(e.rechercher(nom, type));
}

void SmartMarket::equip_onTrierAsc()
{
    Equipement e;
    loadEquipementTable(e.trierParNom("ASC"));
}

void SmartMarket::equip_onTrierDesc()
{
    Equipement e;
    loadEquipementTable(e.trierParNom("DESC"));
}

// ================================================================
// STATS BAR (bottom labels, updated on every table refresh)
// ================================================================
void SmartMarket::updateEquipStatsBar()
{
    Equipement e;
    double val = e.valeurTotaleStock();
    ui->equip_labelValeurStock->setText(
        QString("💰 Valeur totale stock : %1 TND").arg(val, 0, 'f', 2));
}

// ================================================================
// EXPORT TO EXCEL  (UTF-8 BOM CSV – opens correctly in Excel)
// ================================================================
void SmartMarket::equip_onExporterExcel()
{
    const QString filePath = QFileDialog::getSaveFileName(
        this, "Exporter équipements vers Excel", "",
    ...
    out << "ID;Nom;Type;Etat;Quantite;Date Achat;Prix (TND);ID Conference\n";

    Equipement e;
    QSqlQueryModel *model = e.afficher();
    for (int i = 0; i < model->rowCount(); ++i) {
        out << model->index(i, 0).data().toString()                          << ";"
            << model->index(i, 1).data().toString()                          << ";"
            << model->index(i, 2).data().toString()                          << ";"
            << model->index(i, 3).data().toString()                          << ";"
            << model->index(i, 4).data().toString()                          << ";"
            << model->index(i, 5).data().toDate().toString("yyyy-MM-dd")     << ";"
            << QString::number(model->index(i, 6).data().toDouble(),'f',2)   << ";"
    ...
    QMessageBox::information(this, "Export réussi",
                             "Fichier exporté avec succès :\n" + filePath +
                                 "\n\nOuvrez-le avec Excel (File > Open, choisir séparateur ';').");
}

// ================================================================
// STATISTICS DIALOG
// ================================================================
void SmartMarket::equip_onShowStats()
{
    Equipement e;

    // Cost by type
    QMap<QString,double> coutTypes = e.coutParType();

    // Total stock value
    double valStock = e.valeurTotaleStock();

    // Cost by conference (only those that have equipment)
    QSqlQuery q;
    q.exec(
        "SELECT DISTINCT idconference FROM OUSSAMA.equipement "
        "WHERE idconference IS NOT NULL ORDER BY idconference"
        );

    QString msg = "<h3>📊 Statistiques des Équipements</h3><hr>";

    // --- By type ---
    msg += "<b>Coût total par type (prix × quantité) :</b><ul>";
    if (coutTypes.isEmpty()) {
        msg += "<li><i>Aucune donnée</i></li>";
    } else {
        for (auto it = coutTypes.cbegin(); it != coutTypes.cend(); ++it)
            msg += QString("<li><b>%1</b> : %2 TND</li>")
                       .arg(it.key()).arg(it.value(), 0, 'f', 2);
    }
    msg += "</ul>";

    // --- By conference ---
    msg += "<b>Coût total par conférence :</b><ul>";
    bool hasConf = false;
    while (q.next()) {
        hasConf = true;
        int idConf = q.value(0).toInt();
        double cout = e.coutTotalParConference(idConf);
        msg += QString("<li>Conférence <b>#%1</b> : %2 TND</li>")
                   .arg(idConf).arg(cout, 0, 'f', 2);
    }
    if (!hasConf)
        msg += "<li><i>Aucun équipement lié à une conférence</i></li>";
    msg += "</ul>";

    // --- Grand total ---
    msg += QString("<hr><b>💰 Valeur totale du stock : %1 TND</b>")
               .arg(valStock, 0, 'f', 2);

    // ── Équipement le plus cher ─────────────────────────────────
    auto [nomCher, prixCher] = e.equipementLePlusCher();
    msg += QString("<hr><b>🏆 Équipement le plus cher :</b> %1 — <b>%2 TND</b>")
               .arg(nomCher).arg(prixCher, 0, 'f', 2);

    // ── Répartition par état physique ──────────────────────────
    QMap<QString,int> statsEtat = e.statistiquesParEtat();
    msg += "<hr><b>📋 Répartition par état physique :</b><ul>";
    for (auto it = statsEtat.cbegin(); it != statsEtat.cend(); ++it)
        msg += QString("<li><b>%1</b> : %2 équipement(s)</li>")
                   .arg(it.key()).arg(it.value());
    msg += "</ul>";

    // ── Alertes stock ──────────────────────────────────────────
    QList<Equipement> alertes = e.equipementsEnAlerte();
    if (!alertes.isEmpty()) {
        msg += "<hr><b>⚠️ Équipements en alerte :</b><ul>";
        for (const Equipement &eq : alertes) {
            QString couleur = (eq.getQuantite() == 0) ? "red" : "darkorange";
            QString statut  = eq.getStatutStock();
            msg += QString("<li><span style='color:%1'><b>%2</b> (%3) — %4 — %5 unité(s)</span></li>")
                       .arg(couleur, eq.getNom(), eq.getType(),
                            statut, QString::number(eq.getQuantite()));
        }
        msg += "</ul>";
    } else {
        msg += "<hr><span style='color:green'>✅ Aucun équipement en alerte de stock.</span>";
    }

    QMessageBox dlg(this);
    dlg.setWindowTitle("Statistiques Équipements");
...
void SmartMarket::equip_onConnecterArduino()
{
    QStringList ports = arduinoEquip->portsDisponibles();

    if (ports.isEmpty()) {
        QMessageBox::warning(this, "Arduino",
                             "Aucun port série détecté.\n"
                             "Vérifiez que l'Arduino est branché.");
        return;
    }
    ...
    if (!ok || choix.isEmpty()) return;

    QString portName = choix.split(" — ").first().trimmed();

    if (arduinoEquip->connecter(portName)) {
        ui->equip_btnArduino->setText("🔴 Déconnecter Arduino");
        ui->equip_btnArduino->disconnect();
        connect(ui->equip_btnArduino, &QPushButton::clicked,
                this, [this]() {
                    arduinoEquip->deconnecter();
                    ui->equip_btnArduino->setText("🔌 Connecter Arduino");
...
void SmartMarket::equip_onArduinoEvent(int idEquipement)
{
    Equipement e;

    if (!e.decrementerQuantite(idEquipement)) {
        QMessageBox::critical(this, "Arduino — Erreur",
                              QString("Impossible de décrémenter l'équipement ID %1.\n"
                                      "Vérifiez qu'il existe et a du stock.")
                                  .arg(idEquipement));
        return;
    }
    ...
    QSqlQuery q;
    q.prepare(
        "SELECT nom, quantite FROM OUSSAMA.equipement "
    ...
    if (q.next()) {
        ...
        int     qte = q.value(1).toInt();

        if (qte == 0) {
            QMessageBox::critical(this,
                                  "🔴 RUPTURE DE STOCK",
                                  QString("L'équipement '%1' (ID %2) est en rupture de stock !\n"
                                          "Quantité restante : 0")
                                      .arg(nom).arg(idEquipement));
        } else if (qte <= 5) {
            QMessageBox::warning(this,
                                 "🟠 Stock Faible",
                                 QString("L'équipement '%1' (ID %2) a un stock faible.\n"
                                         "Quantité restante : %3")
                                     .arg(nom).arg(idEquipement).arg(qte));
        }
    }
