...
{
    QSqlQuery query;
    query.prepare("INSERT INTO SELIM.PUBLICATION (IDPUBLICATION, TITRE, SOURCE, DOMAINE, DATEPUBLICATION, STATUT, CONTENU) "
...
QSqlQueryModel* Publication::afficher()
{
    ...
    model->setQuery("SELECT IDPUBLICATION, TITRE, SOURCE, DOMAINE, DATEPUBLICATION, STATUT, CONTENU FROM SELIM.PUBLICATION ORDER BY IDPUBLICATION");
...
{
    QSqlQuery query;
    query.prepare("DELETE FROM SELIM.PUBLICATION WHERE IDPUBLICATION = :id");
    query.bindValue(":id", id);
    return query.exec();
}
...
{
    QSqlQuery query;
    query.prepare("UPDATE SELIM.PUBLICATION SET TITRE=:titre, SOURCE=:source, DOMAINE=:domaine, DATEPUBLICATION=:date, STATUT=:statut, CONTENU=:contenu "
...
{
    QSqlQueryModel *model = new QSqlQueryModel();
    QString queryStr = "SELECT IDPUBLICATION, TITRE, SOURCE, DOMAINE, DATEPUBLICATION, STATUT, CONTENU FROM SELIM.PUBLICATION ORDER BY " + critere + " " + ordre;
...
{
    QSqlQueryModel *model = new QSqlQueryModel();
    QString sql = "SELECT IDPUBLICATION, TITRE, SOURCE, DOMAINE, DATEPUBLICATION, STATUT, CONTENU FROM SELIM.PUBLICATION WHERE 1=1";
